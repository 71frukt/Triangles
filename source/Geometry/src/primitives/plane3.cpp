#include "Geometry/primitives/plane3.hpp"
...
Plane3::Plane3(const double A, const double B, const double C, const double D)
{
    ...
    normalized_normality_ = normality.Normalized();
...
Plane3::Plane3(Point3 A, Point3 B, Point3 C)
{
    ...
    normalized_normality_ = (ab ^ ac).Normalized();

    Math::Vector3 rad_vec_0(A);
    D_ = -(rad_vec_0 * normalized_normality_);
}
