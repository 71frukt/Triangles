...
def default_dirs(script_dir:Path):
    return script_dir/"tasks", script_dir/"answers"

def main():
    script_dir = Path(__file__).resolve().parent
    d_tests, d_keys = default_dirs(script_dir)

    ap = argparse.ArgumentParser(description="Run BIN < task.dat and compare stdout with key file.")
    ap.add_argument("-b","--bin", default=str(script_dir/"bin"),
                    help=f"Path to executable (default: {script_dir/'bin'})")
    ap.add_argument("-t","--task-dir", default=str(d_tests),
                    help=f"Directory with input .dat (default: {d_tests})")
    ap.add_argument("-k","--key-dir", default=str(d_keys),
                    help=f"Directory with expected .dat (default: {d_keys})")
    args = ap.parse_args()

    # Make paths absolute FROM CWD
    bin_path = resolve_from_cwd(args.bin)
    task_dir = resolve_from_cwd(args.task_dir)
    ...
    tasks = sorted(p for p in task_dir.glob("*.dat"))
    if not task_dir.is_dir():
        print(color("ERROR: Test dir not found: ","red")+str(task_dir)); sys.exit(2)
    if not key_dir.is_dir():
        print(color("ERROR: Key dir not found: ","red")+str(key_dir)); sys.exit(2)
    if not tasks:
        print(color(f"No task files in {task_dir}","yellow")); sys.exit(2)

    print(color(f"BIN: {bin_path}", "dim"))
    print(color(f"TEST_DIR: {task_dir}", "dim"))
